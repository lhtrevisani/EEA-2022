...
Observando los gráficos, s eobserva que existe cierta correlación entre entre el peso y la altura (x,5 para las personas de género masculino, x.8 para las de género femenino.
...
En estos gráficos, se puede observar que aquellos que suelen pasar hambre registran un menor consumo de verduras y hortalizas.
...
INTERPRETACION COEFICIENTES (individial, conjunta e interacción)
...
```{r}
tidy(anova(modelo_simple))
```

En caso de detectar que existen categorías no significativas de la variable consumoSemanalSnacks evaluar si la variable es significativa en su conjunto y, en caso afirmativo, proponer una redefinición de las mismas que permita obtener una mayor proporción de categorías significativas individualmente.

Considerando que las categorías de la consumoSemanalSnacks no significativas hacen referencia al consumo de verduras más de una vez por día, se va a consolidar en una única y se van a volver a estimar los coeficientes.

Luego, analizar si existen cambios en la variabilidad explicada por el modelo.
